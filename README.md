...
| `XMONITOR_DB_MIGRATIONS_FROM_FILE` | Optional | Limits in-Lambda migration bootstrap to files at or after the named SQL migration (for least-privilege app roles that should not re-run privileged bootstrap migrations). |
...
- Keep `xmonitor-xapi-priority-collector-15m` and `xmonitor-xapi-discovery-collector-30m` enabled for normal production ingestion.
...
- Script updates the canonical repo omit config (`config/xmonitor/omit-handles.json`).
...
- Add `--update-lambda-env --aws-profile zodldashboard --aws-region us-east-1` to also update live Lambda env var `XMONITOR_INGEST_OMIT_HANDLES` on the backend and both collector functions.
