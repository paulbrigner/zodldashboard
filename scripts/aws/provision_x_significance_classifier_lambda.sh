...
#   SIGNIFICANCE_LLM_MODEL=deepseek-v4-flash-0731
#   SIGNIFICANCE_LLM_FALLBACK_MODELS=z-ai-glm-5-3-flash
#   SIGNIFICANCE_LLM_API_KEY=...               # fallback from DB secret embedding_api_key/venice_api_key
#   SIGNIFICANCE_LLM_TEMPERATURE=0
#   SIGNIFICANCE_LLM_MAX_TOKENS=900
#   SIGNIFICANCE_LLM_TIMEOUT_MS=30000
#   SIGNIFICANCE_LLM_MAX_ATTEMPTS=2
#   SIGNIFICANCE_LLM_INITIAL_BACKOFF_MS=1000
#   SIGNIFICANCE_DISABLE_THINKING=true
#   SIGNIFICANCE_BATCH_SIZE=4
#   SIGNIFICANCE_MAX_POSTS_PER_RUN=64
#   SIGNIFICANCE_MAX_ATTEMPTS=10
#   SIGNIFICANCE_LEASE_SECONDS=300
#   SIGNIFICANCE_VERSION=ai_v3
#   SIGNIFICANCE_LAMBDA_SAFETY_MARGIN_MS=10000
#   LAMBDA_TIMEOUT=240
...
SIGNIFICANCE_LLM_MODEL="${SIGNIFICANCE_LLM_MODEL:-deepseek-v4-flash-0731}"
SIGNIFICANCE_LLM_FALLBACK_MODELS="${SIGNIFICANCE_LLM_FALLBACK_MODELS-z-ai-glm-5-3-flash}"
SIGNIFICANCE_LLM_API_KEY="${SIGNIFICANCE_LLM_API_KEY:-}"
SIGNIFICANCE_LLM_TEMPERATURE="${SIGNIFICANCE_LLM_TEMPERATURE:-0}"
SIGNIFICANCE_LLM_MAX_TOKENS="${SIGNIFICANCE_LLM_MAX_TOKENS:-900}"
SIGNIFICANCE_LLM_TIMEOUT_MS="${SIGNIFICANCE_LLM_TIMEOUT_MS:-30000}"
SIGNIFICANCE_LLM_MAX_ATTEMPTS="${SIGNIFICANCE_LLM_MAX_ATTEMPTS:-2}"
SIGNIFICANCE_LLM_INITIAL_BACKOFF_MS="${SIGNIFICANCE_LLM_INITIAL_BACKOFF_MS:-1000}"
SIGNIFICANCE_DISABLE_THINKING="${SIGNIFICANCE_DISABLE_THINKING:-true}"
SIGNIFICANCE_BATCH_SIZE="${SIGNIFICANCE_BATCH_SIZE:-4}"
SIGNIFICANCE_MAX_POSTS_PER_RUN="${SIGNIFICANCE_MAX_POSTS_PER_RUN:-64}"
SIGNIFICANCE_MAX_ATTEMPTS="${SIGNIFICANCE_MAX_ATTEMPTS:-10}"
SIGNIFICANCE_LEASE_SECONDS="${SIGNIFICANCE_LEASE_SECONDS:-300}"
SIGNIFICANCE_VERSION="${SIGNIFICANCE_VERSION:-ai_v3}"
SIGNIFICANCE_LAMBDA_SAFETY_MARGIN_MS="${SIGNIFICANCE_LAMBDA_SAFETY_MARGIN_MS:-10000}"

LAMBDA_TIMEOUT="${LAMBDA_TIMEOUT:-240}"
...
ENV_JSON="$(
  ...
  SIGNIFICANCE_LLM_FALLBACK_MODELS="$SIGNIFICANCE_LLM_FALLBACK_MODELS" \
  SIGNIFICANCE_LLM_API_KEY="$SIGNIFICANCE_LLM_API_KEY" \
  ...
  SIGNIFICANCE_DISABLE_THINKING="$SIGNIFICANCE_DISABLE_THINKING" \
  ...
  SIGNIFICANCE_LAMBDA_SAFETY_MARGIN_MS="$SIGNIFICANCE_LAMBDA_SAFETY_MARGIN_MS" \
  python3 - <<'PY'
import json, os
print(json.dumps({
  "Variables": {
    ...
    "XMON_SIGNIFICANCE_LLM_FALLBACK_MODELS": os.environ["SIGNIFICANCE_LLM_FALLBACK_MODELS"],
    ...
    "XMON_SIGNIFICANCE_DISABLE_THINKING": os.environ["SIGNIFICANCE_DISABLE_THINKING"],
    ...
    "XMON_SIGNIFICANCE_LAMBDA_SAFETY_MARGIN_MS": os.environ["SIGNIFICANCE_LAMBDA_SAFETY_MARGIN_MS"],
  }
}))
PY
)"
...
echo "  aws --region $AWS_REGION lambda invoke --function-name $LAMBDA_FUNCTION_NAME --payload '{\"max_posts_per_run\":64}' /tmp/xmon_significance_classifier_test.json && cat /tmp/xmon_significance_classifier_test.json"
