...
import { upsertWindowSummaries } from "@/lib/xmonitor/repository";
import { parseBatchItems, parseWindowSummaryUpsert } from "@/lib/xmonitor/validators";
import type { BatchUpsertResult, WindowSummaryUpsert } from "@/lib/xmonitor/types";
...
export async function POST(request: Request) {
  ...
  const validItems: WindowSummaryUpsert[] = [];
  ...
  parsedBatch.items.forEach((item, index) => {
    const parsed = parseWindowSummaryUpsert(item);
    if (!parsed.ok) {
  ...
  try {
    ...
    const dbResult = await upsertWindowSummaries(validItems);
    ...
    return jsonError(error instanceof Error ? error.message : "failed to upsert window summaries", 503);
  }
}
