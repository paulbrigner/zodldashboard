...
export async function POST(request: Request) {
  ...
  const validItems: MetricsSnapshotUpsert[] = [];
  ...
  parsedBatch.items.forEach((item, index) => {
    const parsed = parseMetricsSnapshotUpsert(item);
    if (!parsed.ok) {
  ...
  try {
    ...
    const dbResult = await upsertMetricSnapshots(validItems);
    ...
    return jsonError(error instanceof Error ? error.message : "failed to upsert metric snapshots", 503);
  }
}
