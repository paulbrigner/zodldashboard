...
import { upsertPosts } from "@/lib/xmonitor/repository";
import { parseBatchItems, parsePostUpsert } from "@/lib/xmonitor/validators";
import type { BatchUpsertResult, PostUpsert } from "@/lib/xmonitor/types";
...
export async function POST(request: Request) {
  ...
  parsedBatch.items.forEach((item, index) => {
    const parsed = parsePostUpsert(item);
    if (!parsed.ok) {
  ...
  try {
    ...
    const dbResult = await upsertPosts(validItems);
    ...
    return jsonError(error instanceof Error ? error.message : "failed to upsert posts", 503);
  }
}
