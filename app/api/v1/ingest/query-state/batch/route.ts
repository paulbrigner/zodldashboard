...
import { upsertIngestQueryCheckpoints } from "@/lib/xmonitor/repository";
import { parseBatchItems, parseIngestQueryCheckpointUpsert } from "@/lib/xmonitor/validators";
import type { BatchUpsertResult, IngestQueryCheckpointUpsert } from "@/lib/xmonitor/types";
...
export async function POST(request: Request) {
  ...
  const validItems: IngestQueryCheckpointUpsert[] = [];
  ...
  parsedBatch.items.forEach((item, index) => {
    const parsed = parseIngestQueryCheckpointUpsert(item);
    if (!parsed.ok) {
  ...
  try {
    ...
    const dbResult = await upsertIngestQueryCheckpoints(validItems);
    ...
    return jsonError(error instanceof Error ? error.message : "failed to upsert query checkpoints", 503);
  }
}
